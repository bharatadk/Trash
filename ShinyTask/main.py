...
def save_task_to_database(filenames, date='0', time='0',  filetype="pdf",email="example@gmail.com"):
...
app_ui = ui.page_fluid(
   ...
   ui.input_text("time1", "Enter Time (24 hr format) ", value="8:30"),
    ...
    ui.input_file("file1", "Choose a file to upload:",accept="application/pdf", multiple=False),
    ui.output_text_verbatim("file_content"),
    ui.h3("Result Table:"),
    ui.output_text_verbatim("current_time1", placeholder=True),
    ui.h6("[ task-id ] - [ date ] - - - -  [ time ] - [ STATUS ]  - - - -  - - [ RESULT ]"),
...
def pdf_to_text(filenames):
    try:
        pdf_document = fitz.open(os.path.join(os.environ['UPLOAD_FOLDER'],filenames[0]))
        text = ""
        count=1
